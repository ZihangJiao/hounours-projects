...
for idx, (in_seq, tgt_seq,word_time_distribution, in_len, tgt_len) in enumerate(test_dataloader):
    ...
    with torch.no_grad():
        ...
        # unpacked_out_enc, enc_mask, enc_h, enc_c = model.encoder(in_seq, in_len)
...
# arr = target_arr[0]
arr = output_arr[0]
f = open("head_motion.txt","w")
for i in range(len(target_arr)):
    # print(arr[i][0].data.tolist())
    # print(arr[i][0].string())
    f.write(str(arr[i][0].data.tolist()) + " " + str(arr[i][1].data.tolist()) + " " + str(arr[i][2].data.tolist()) + " " + str(arr[i][3].data.tolist()) + '\n')

arr = target_arr[0]
f = open("head_motion_target.txt","w")
for i in range(len(target_arr)):
    # print(arr[i][0].data.tolist())
    # print(arr[i][0].string())
    f.write(str(arr[i][0].data.tolist()) + " " + str(arr[i][1].data.tolist()) + " " + str(arr[i][2].data.tolist()) + " " + str(arr[i][3].data.tolist()) + '\n')
